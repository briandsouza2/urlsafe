...
def timed_test():
    ...
    contents = urllib.request.urlopen(f"http://localhost:5000/urlinfo/1/hostname01_80/{rand_str}").read()
...
def sanity_test():
    ...
    contents = urllib.request.urlopen(f"http://localhost:5000/urlinfo/1/hostname01_80/{query}").read()
    response = json.loads(contents)
